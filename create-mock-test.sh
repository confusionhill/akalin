...
if [ -z "$TOKEN" ]; then
  echo -e "Error: TOKEN environment variable is required"
  echo "Please login first and copy your token:"
  echo "  export TOKEN=\"\$(cat ~/.llm_eval_token 2>/dev/null || echo '')\""
  echo "Or pass it directly:"
  echo "  TOKEN=\"your-token-here\" ./create-mock-test.sh"
  exit 1
fi
...
TC3_RESPONSE=$(curl -s -X POST "${API_BASE_URL}/projects/${PROJECT_ID}/test-cases" \
  ...
  -H "Authorization: Bearer ${TOKEN}" \
  -d "{
    \"input_prompt\": \"Write a professional email response to the following customer complaint:\\n\\nDear Support Team,\\n\\nI purchased your product two weeks ago and it has been completely broken ever since. I expected better quality from your company.\",
...
TC4_RESPONSE=$(curl -s -X POST "${API_BASE_URL}/projects/${PROJECT_ID}/test-cases" \
  ...
  -H "Authorization: Bearer ${TOKEN}" \
  -d "{
    \"input_prompt\": \"Explain what a blockchain is and why it's important for cryptocurrency. Use simple terms that a non-technical person would understand.\",
